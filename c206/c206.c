...
void DLL_DeleteFirst(DLList *list) {
    ...
    DLLElementPtr newElemPtr = list->firstElement->nextElement;
    if(newElemPtr != NULL) {
        newElemPtr->previousElement = NULL;
    }
...
void DLL_DeleteLast(DLList *list) {
    ...
    DLLElementPtr lastElemPtr = list->lastElement;
...
void DLL_DeleteAfter(DLList *list) {
    if (list->activeElement != NULL && list->activeElement != list->lastElement) {
        DLLElementPtr nextElemPtr = list->activeElement->nextElement;
        list->activeElement->nextElement = nextElemPtr->nextElement;
        if (nextElemPtr->nextElement != NULL) {
            nextElemPtr->nextElement->previousElement = list->activeElement;
        }
        if (nextElemPtr == list->lastElement) {
...
void DLL_DeleteBefore(DLList *list) {
    ...
    {
        DLLElementPtr prevElemPtr = list->activeElement->previousElement;
        list->activeElement->previousElement = prevElemPtr->previousElement;
        if (prevElemPtr->previousElement != NULL) {
            prevElemPtr->previousElement->nextElement = list->activeElement;
        }
        if (prevElemPtr == list->firstElement) {
